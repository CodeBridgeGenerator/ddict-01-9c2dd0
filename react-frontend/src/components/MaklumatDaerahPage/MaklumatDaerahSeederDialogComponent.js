...
import seederData from "./maklumatDaerah.seeder.json";
...
const MaklumatDaerahSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("maklumatDaerah").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(MaklumatDaerahSeederDialogComponent);
