...
import maklumatDaerahFakerFactory from "./maklumatDaerahFakerFactory";
...
const MaklumatDaerahFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = maklumatDaerahFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("maklumatDaerah").create(elem));
...
export default connect(mapState, mapDispatch)(MaklumatDaerahFakerDialogComponent);
