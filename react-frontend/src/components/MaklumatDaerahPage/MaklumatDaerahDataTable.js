...
const MaklumatDaerahDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.namaDaerah}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "maklumatDaerah"});
...
<Column field="namaDaerah" header="Nama Daerah" body={pTemplate0} filter={selectedFilterFields.includes("namaDaerah")} hidden={selectedHideFields?.includes("namaDaerah")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload MaklumatDaerah Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search MaklumatDaerah" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default MaklumatDaerahDataTable;