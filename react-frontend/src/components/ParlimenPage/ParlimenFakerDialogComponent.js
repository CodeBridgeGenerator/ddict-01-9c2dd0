...
import parlimenFakerFactory from "./parlimenFakerFactory";
...
const ParlimenFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = parlimenFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("parlimen").create(elem));
...
export default connect(mapState, mapDispatch)(ParlimenFakerDialogComponent);
