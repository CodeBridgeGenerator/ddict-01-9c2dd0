...
import dunFakerFactory from "./dunFakerFactory";
...
const DunFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dunFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dun").create(elem));
...
export default connect(mapState, mapDispatch)(DunFakerDialogComponent);
