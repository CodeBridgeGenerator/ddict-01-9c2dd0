...
const DunCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            namaDun: _entity?.namaDun,
kodDun: _entity?.kodDun,
kodParlimen: _entity?.kodParlimen,
        ...
        const result = await client.service("dun").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info dun updated successfully" });
    ...
    return (
        <Dialog header="Edit Dun" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="dun-edit-dialog-component">
